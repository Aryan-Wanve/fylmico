    ...
    <DashboardPanel title="Upcoming (Next 7 Days)">
      {upcoming.length === 0 ? (
        ...
        <div className="grid">
          {upcoming.map((event) => (
            <div
              className="flex items-start gap-3 border-b border-black/5 px-6 py-3.5 last:border-b-0"
              key={event.id}
            >
              <span
              ...
              {event.location ? (
                <span className="shrink-0 rounded-md bg-black/[0.03] px-2 py-1 text-xs font-semibold text-[#5f667d]">
                  {event.location}
                </span>
              ) : null}
            </div>
