      ...
      {tasks.map((task) => {
        ...
        return (
          ...
          >
            ...
            />
            <span
              className={`flex-1 text-sm font-semibold ${done ? "text-[#667085] line-through dark:text-[#878ca0]" : "text-[#11142c] dark:text-[#f1f2f8]"}`}
            >
              {task.title}
            </span>
            <span className="shrink-0 text-xs font-medium text-[#667085] dark:text-[#878ca0]">
              {assigneeLabel}
            </span>
          </div>
        );
