  ...
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
      {templates.map((template) => (
        <article
          className="flex flex-col rounded-2xl border border-black/[0.06] bg-white p-5 shadow-[0_1rem_3rem_rgba(53,45,124,0.05)]"
          key={template.id}
        >
          <span className="grid h-11 w-11 place-items-center rounded-xl bg-amber-50 text-amber-600">
            <LayoutTemplate className="h-5 w-5" />
          </span>
          <strong className="mt-3 text-sm font-bold text-[#11142c]">
            {template.name}
          </strong>
          <p className="mt-1 flex-1 text-sm leading-relaxed text-[#5f667d]">
