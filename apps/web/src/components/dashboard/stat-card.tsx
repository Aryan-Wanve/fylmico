...
export function StatCard({
  ...
  return (
    <article className="flex min-w-0 items-center gap-4 rounded-2xl border border-black/[0.06] bg-white p-5 shadow-[0_1rem_3rem_rgba(53,45,124,0.05)]">
      <div
      ...
      <div className="min-w-0 flex-1">
        <strong className="block text-sm font-semibold text-[#5f667d]">
          {title}
        </strong>
        <span className="mt-1 block text-2xl font-black text-[#11142c]">
          {value}
        </span>
        <p
          className={`mt-1 text-xs font-semibold ${noteTone === "positive" ? "text-emerald-600" : "text-[#8a90a3]"}`}
        >
          {note}
        </p>
        {extra}
