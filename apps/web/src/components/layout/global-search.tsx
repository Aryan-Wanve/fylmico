  ...
  return (
    <div className="relative w-full max-w-md" ref={containerRef}>
      <div className="flex h-11 w-full items-center gap-2.5 rounded-xl border border-black/10 bg-white px-3.5 dark:border-white/10 dark:bg-[#171a28]">
        ...
        <input
          className="h-full flex-1 bg-transparent text-sm text-[#12142b] outline-none placeholder:text-[#9296a4] dark:text-[#f1f2f8] dark:placeholder:text-[#878ca0]"
          onChange={(event) => {
      ...
      {open && trimmed ? (
        <div className="absolute top-full left-0 z-30 mt-2 w-full overflow-hidden rounded-xl border border-black/10 bg-white shadow-[0_1rem_3rem_rgba(53,45,124,0.12)] dark:border-white/10 dark:bg-[#171a28]">
          {results.length === 0 ? (
            <p className="px-4 py-3 text-sm text-[#667085] dark:text-[#878ca0]">
              ...
              {results.map((result) => (
                <li key={result.id}>
                  <button
                    ...
                    type="button"
                  >
                    <result.icon className="h-4 w-4 shrink-0 text-[#667085] dark:text-[#878ca0]" />
